<template>
  <div class="p-1 rounded-3xl bg-card border shadow-2xl backdrop-blur-xl bg-card/80">
    <div class="p-4 space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        >
          <option v-for="m in MODES" :key="m.id" :value="m.id">
            {{ $t(`common.${m.id === 'text2speech' ? 'audio' : m.id === 'image-edit' ? 'edit' : m.id === 'text2video' ? 'video' : 'image'}`) }}
          </option>
        </select>
        ...
        >
          <option v-for="(m, id) in availableModels" :key="id" :value="id">
            {{ m.name }}
          </option>
        </select>
        ...
        <!-- Resolution Selector -->
        <select 
          v-if="mode === 'text2image' || mode === 'image-edit' || mode === 'text2video'"
        ...
        >
          <option v-for="res in availableResolutions" :key="res.id" :value="res.id">
            {{ res.label }}
          </option>
        </select>
        ...
        <button 
          ...
          :class="{ 'bg-primary/10 text-primary': showParams }"
        >
      ...
      <Transition name="slide-down">
        <div v-if="showParams" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-muted/20 rounded-2xl border-t border-white/5">
          <div v-for="(cfg, key) in filteredParams" :key="key" class="space-y-1.5">
            <div class="flex justify-between items-center px-1">
              <label class="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">{{ $t(`common.${key}`) || key }}</label>
              <span class="text-xs font-mono font-bold">{{ params[key] }}</span>
            </div>
            <input 
              ...
              :step="cfg.step || 1"
              :value="params[key]"
              @input="params[key] = ($event.target as HTMLInputElement).valueAsNumber"
      ...
      <div v-if="mode === 'image-edit' || mode === 'image2video'" class="flex gap-2">
         <div v-for="(img, idx) in sourceImages" :key="idx" class="relative w-16 h-16 group">
            <img :src="img.url" class="w-full h-full object-cover rounded-xl border" />
            <button @click="removeSourceImage(idx)" class="absolute -top-2 -right-2 bg-destructive text-destructive-foreground rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <X class="w-3 h-3" />
            </button>
         </div>
         <label v-if="sourceImages.length < (selectedModel?.imageInput?.maxItems || 1)" class="w-16 h-16 flex flex-col items-center justify-center bg-muted rounded-xl border border-dashed hover:bg-muted/80 cursor-pointer transition-colors">
      ...
      <div class="relative group">
        ...
        <div class="absolute bottom-4 right-4 flex items-center gap-2">
          <span v-if="isGenerating" class="text-[10px] font-mono text-muted-foreground animate-pulse">{{ timer.toFixed(2) }}s</span>
          <button 
            @click="generate"
          ...
          >
            ...
            {{ isGenerating ? $t('common.loading') : $t('common.generate') }}
          </button>
        </div>
      </div>
    </div>
...
import { Sparkles, Loader2, Settings2, Plus, X, BrushCleaning } from 'lucide-vue-next';
...
const { mode, selectedModelId, availableModels, params, selectedModel, selectedResolution, availableResolutions } = storeToRefs(modelsStore);
...
const isValid = computed(() => {
  ...
  if (mode.value === 'image-edit') return prompt.value.length > 0 && sourceImages.value.length > 0;
  return prompt.value.length > 0;
});

const filteredParams = computed(() => {
  if (!selectedModel.value?.params) return {};
  // Filter out text, width, height (if we want to use preset select instead)
  ...
  try {
    ...
    if (model.imageInput) {
      if (model.imageInput.type === 'single' && sourceImages.value[0]) {
        payload[model.imageInput.field] = sanitizeBase64(sourceImages.value[0].b64);
      } else if (sourceImages.value.length > 0) {
        payload[model.imageInput.field] = sourceImages.value.map(img => sanitizeBase64(img.b64));
      }
    }

    const endpoint = mode.value === 'text2video' ? model.endpoints.text2video : model.endpoint;
    
    // Detailed logging as requested
    const redactor = (key: string, value: any) => {
      if (typeof value === 'string' && (key.includes('b64') || key === 'image' || key === 'img_b64' || key === 'image_b64s')) {
        return value.length > 50 ? `${value.substring(0, 50)}... [truncated]` : value;
      }
      return value;
    };
    ...
    configStore.addLog({ type: mode.value, message: `Generación exitosa: ${model.name}` });
    configStore.refreshQuota();
  } catch (err: any) {
    console.error(err);
    toast.error(`Error: ${err.message}`);
    configStore.addLog({ type: mode.value, message: `Error: ${err.message}`, error: true });
  } finally {
