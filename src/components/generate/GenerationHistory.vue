<template>
  <div v-if="sessionItems.length > 0" class="space-y-4 max-h-[70vh] overflow-y-auto">
    <div
      v-for="item in sessionItems"
      :key="item.id"
      class="flex gap-4 p-4 bg-muted/10 rounded-2xl border"
    >
      <!-- Prompt Section -->
      ...
      <div class="relative max-w-xs w-full flex-shrink-0">
        <div v-if="item.type === 'image' && blobUrls[item.id]" class="relative">
          <img
        ...
        <div v-else-if="item.type === 'video' && blobUrls[item.id]" class="relative">
          <video
            :src="blobUrls[item.id]"
            class="w-full max-h-48 object-contain rounded-xl shadow-lg"
            controls
        ...
        <div v-else-if="item.type === 'audio' && blobUrls[item.id]" class="flex flex-col items-center gap-2 p-4 bg-muted/20 rounded-xl">
        ...
        <div v-else class="w-full h-48 bg-muted/20 rounded-xl flex items-center justify-center">
