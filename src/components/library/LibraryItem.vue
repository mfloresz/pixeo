<template>
  ...
  <div class="group relative bg-muted/30 rounded-2xl overflow-hidden border hover:border-primary/50 transition-all shadow-sm hover:shadow-xl hover:shadow-primary/5" style="content-visibility: auto; contain-intrinsic-size: 0 250px;">
    <div ref="itemRef" class="aspect-square bg-muted flex items-center justify-center relative overflow-hidden">
      <template v-if="isVisible && blobUrl">
        <img v-if="item.type === 'image'" :src="blobUrl" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" decoding="async" />
        <video v-else-if="item.type === 'video'" :src="blobUrl" class="w-full h-full object-cover" muted loop preload="none" @mouseenter="$event.target.play()" @mouseleave="$event.target.pause()" />
      ...
      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-sm">
        <Tooltip>
          <TooltipTrigger as-child>
            <button @click="$emit('zoom', item)" class="p-2 bg-white/20 hover:bg-white/30 rounded-full text-white transition-colors">
...
onMounted(() => {
  const { stop } = useIntersectionObserver(
    ...
    ([{ isIntersecting }], observerElement) => {
      if (isIntersecting && !blobLoaded.value) {
